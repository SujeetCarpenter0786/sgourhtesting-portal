...
$res = mysqli_query($conn, "SELECT * FROM `pantxn`  WHERE status='pending'");
if (isset($_POST['delete']) && $_POST['delete'] == "ahkwebsolutions") {
    $id = base64_decode($_POST['id']);
    $del = mysqli_query($conn, "DELETE FROM `pantxn` WHERE id='$id'");
    if ($del) {
    ?>
        <script>
            $(function() {
...
if (isset($_POST['approve']) && $_POST['approve'] == "ahkwebsolutions" && $_POST['accesspin'] != NULL) {
    ...
    if($ahkweb['accesspin'] == $accesspin){
        ...
        $newamount = $sddata['panwallet'] + $amount;
        $approve_sql = mysqli_query($conn,"UPDATE `pantxn` SET `status`='APPROVED'  WHERE emailid='$emailid'");
        $credit = mysqli_query($conn,"UPDATE `usertable` SET panwallet='$newamount' WHERE emailid='$emailid'");
...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">PAN WALLET Payment Requests</h1>
                </div><!-- /.col -->
