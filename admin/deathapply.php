...
if (isset($_POST['ahkweb']) && $_POST['ahkweb'] == "ahkwebsolutions") {
	...
	$sres = mysqli_query($conn, "SELECT * FROM `dportal` WHERE state='$state' ");
}


if(isset($_POST['d_verify']) && $_POST['d_verify'] == "ahkweb"){
    $registration_no = "D-2022: 9-90347-00".rand(0000, 9999) ;
    $name = mysqli_real_escape_string($conn,$_POST['name']);
    $aadhar = mysqli_real_escape_string($conn,$_POST['aadhar']);
    $gender = mysqli_real_escape_string($conn,$_POST['gender']);
    $date_of_death = mysqli_real_escape_string($conn,$_POST['date_of_death']);
    $dod_word = mysqli_real_escape_string($conn,$_POST['dod_word']);
	$dod_age = mysqli_real_escape_string($conn,$_POST['dod_age']);
	$death_place = mysqli_real_escape_string($conn,$_POST['death_place']);
    ...
    $fee = $ahkweb['death_fee'];
    $debit_fee = $wallet_amount - $fee;
    if($wallet_amount > $fee){
        $ires = mysqli_query($conn,"INSERT INTO `dmanual`(`name`, `aadharno`, `dob`, `dobwords`, `placeofbirth`, `mname`, `maadhar`, `fname`, `faadhar`, `birthaddress`, `permanentaddress`, `regno`, `dateofregister`, `dateofissue`, `updatedate`, `username`, `gender`, `state`, `hospital`, `agee`, `hwname`, `hwaadhar`) VALUES ('$name','$aadhar','$date_of_death','$dod_word','$death_place','$mother_name','$mother_aadhar','$father_name','$father_aadhar','$death_address','$permanent_address','$registration_no','$register_date','$dateof_issue','$dateof_update','$username','$gender','$state','$hospital_id','$dod_age','$hus_name','$hus_aadhar')");
		// Debit Amt
    ...
    if($ires){
        ...
        <script>
            $(function(){
                Swal.fire(
                    'Success',
                    'Death Application Submitted Successfully!',
                    'success'
                )
            });
        </script>
				...
				<div class="col-md-12">
					...
					?>
						...
						<div class="card card-default">
							...
							<!-- form start -->
							<form method="post" action="" autocomplete="off" enctype="multipart/form-data">
								<input type="hidden" name="d_verify" value="ahkweb">
								<div class="card-body row">
									...
									<div class="form-group col-md-6">
										<label for="date_of_death">मृत्यु की तिथि / Date of death <span class="text-danger">*</span></label>
										<div class="input-group date" id="d_date_of_death" data-target-input="nearest">
											<input type="text" class="form-control datetimepicker-input" data-target="#d_date_of_death" id="date_of_death" name="date_of_death" placeholder="PLACE OF BIRTH" value="" required="">
											<div class="input-group-append" data-target="#d_date_of_death" data-toggle="datetimepicker">
									...
									<div class="form-group col-md-6">
										<label for="dod_word">मृत्यु की तिथि / Date of death(IN WORD)</label>
										<input type="text" class="form-control" id="dod_word" name="dod_word" placeholder="Ex: FIFTEENH-JULY-TWO THOUSAND ELEVEN" value="">
									</div>

									<div class="form-group col-md-6">
										<label for="dod_age"> मृतक की उम्र / AGE OF DECEASED</label>
										<input type="text" class="form-control" name="dod_age" placeholder="AGE OF DECEASED">
									</div>

									<div class="form-group col-md-12">
										<label for="date_of_death">मृत्यु का स्थान / Place of Death <span class="text-danger">*</span></label>
										<input type="text" class="form-control" id="death_place" name="death_place" placeholder="PLACE OF BIRTH" value="" required="">
									</div>

									<div class="form-group col-md-6">
										<label>पति / पत्नी का नाम / NAME OF HUSBAND / WIFE</label>
										<input type="text" required class="form-control" name="hus_name" aria-describedby="emailHelp" placeholder="FNAME OF HUSBAND / WIFE">
									</div>
									<div class="form-group col-md-6">
										<label for="exampleInputPassword1">पति / पत्नी का आधार संख्या / HUSBAND / WIFE AADHAAR NO</label>
										<input type="text" maxlength="12" class="form-control" name="hus_aadhar" placeholder="HUSBAND / WIFE AADHAAR NO">
									</div>
									...
									<div class="form-group col-md-6">
										<label for="permanent_address">मृतक का स्थायी पता / Permanent Address <span class="text-danger">*</span></label>
									...
									<div class="form-group col-md-6">
										...
										<input type="text" class="form-control" id="death_address" name="death_address" placeholder="ADDRESS OF BIRTH" value="" required="">
									</div>
									...
									<div class="form-group col-md-6">
										...
										<div class="input-group date" id="d_register_date" data-target-input="nearest">
											<input type="text" class="form-control datetimepicker-input" data-target="#d_register_date" id="register_date" name="register_date" placeholder="PLACE OF BIRTH" value="05-11-2024 " required="">
...
<script>
	//Date picker
	$('#d_date_of_death').datetimepicker({
		format: 'L',
		format: 'DD-MM-YYYY'
	});
